...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        # Dimension per head (common choice: d_k = d_q = d_v)
        d_head = d_model // num_attention_heads
        self.d_head = d_head
        
        # Create attention heads
        self.heads = nn.ModuleList(
            [AttentionHead(d_model, d_head, d_head, d_head) for _ in range(num_attention_heads)]
        )
        
        # Final linear layer to project concatenated output back to d_model
        self.output_linear = nn.Linear(num_attention_heads * d_head, d_model)
    ...
    def forward(self, hidden_state):
        ...
        """
        # Raise error if d_model not divisible by number of heads
...
class FeedForward(nn.Module):
    ...
    def __init__(self, d_model: int, intermediate_size: int):
        ...
        # First linear projection: d_model -> intermediate_size
        self.linear_1 = nn.Linear(d_model, intermediate_size)
        
        # Second linear projection: intermediate_size -> d_model
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Multi-head attention with pre-LN and residual
        residual = x
        hidden_state = self.layer_norm_1(x)
        hidden_state = self.attention(hidden_state)
        x = residual + hidden_state  # Residual connection
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        # Combine token and position embeddings and apply layer normalization
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        # Embeddings layer (token + positional embeddings)
...
class ClassificationHead(nn.Module):
    ...
    def __init__(self, d_model: int, num_classes: int, dropout_prob: float):
        ...
        # Linear layer to map hidden states to class logits
...
class TransformerForSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        x = self.transformer_encoder(input_ids)  # shape: (batch_size, seq_len, d_model)
